...
// Create a union type for job listings
export type AnyJobListing = JobListing | EnhancedJobListing;

// Helper functions for user engagement badges
export const isNew = (job: AnyJobListing): boolean => {
  return new Date().getTime() - new Date(job.postedDate).getTime() < 24 * 60 * 60 * 1000;
};

export const isExpiringSoon = (job: AnyJobListing): boolean => {
  if (!job.expiresAt) return false;
  return new Date(job.expiresAt).getTime() - new Date().getTime() < 3 * 24 * 60 * 60 * 1000;
};

export const isVerified = (job: AnyJobListing): boolean => {
