...
function startJobSpyBridge() {
  ...
  pythonProcess.on('close', (code) => {
    if (code !== 0) {
      log(`Python check failed with code ${code}`);
      process.exit(1);
    }
    ...
    const pipProcess = spawn('python3', ['-m', 'pip', 'install', '-r', requirementsPath]);
    ...
    pipProcess.on('close', (code) => {
      ...
      const bridgeProcess = spawn('python3', [bridgeScriptPath], {
        env: {
          ...process.env,
