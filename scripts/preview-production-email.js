...
async function sendProductionPreview() {
  const SENDGRID_API_KEY = process.env.SENDGRID_API_KEY;
  const SENDGRID_FROM_EMAIL = process.env.SENDGRID_FROM_EMAIL;
  const SENDGRID_FROM_NAME = process.env.SENDGRID_FROM_NAME;
  const MONGODB_URI = process.env.MONGODB_URI;
  const MONGODB_DB = process.env.MONGODB_DB;
  const PREVIEW_EMAIL = 'yotamt@gmail.com';

  if (!SENDGRID_API_KEY || !SENDGRID_FROM_EMAIL || !MONGODB_URI) {
  ...
  console.log('\n📧 Sending PRODUCTION Preview Email\n');
  console.log('This is EXACTLY what your subscribers will receive!\n');
  ...
  // Generate email HTML (same as production)
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #1f2937; margin: 0; padding: 0; background-color: #f9fafb;">
  ...
  // Send exactly as production would
  const msg = {
    to: PREVIEW_EMAIL,
    from: {
  ...
  console.log('✅ SUCCESS! Production preview email sent!\n');
  console.log('📧 Email Details:');
  console.log(`   To: ${PREVIEW_EMAIL}`);
  console.log(`   From: ${SENDGRID_FROM_NAME} <${SENDGRID_FROM_EMAIL}>`);
  console.log(`   Subject: 🔔 ${jobs.length} New Remote Jobs This Week`);
  console.log(`   Jobs included: ${jobs.length}`);
  console.log('\n🎉 Check your inbox! This is EXACTLY what subscribers will receive.\n');
}

sendProductionPreview().catch(console.error);
