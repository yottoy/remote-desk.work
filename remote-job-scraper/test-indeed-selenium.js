...
/**
 * Test the Indeed Selenium scraper
 */
async function testIndeedSeleniumScraper() {
  try {
    logger.info('Testing IndeedSelenium Scraper');
    
    // Initialize the scraper
    const scraper = new IndeedSeleniumScraper();
    ...
    const resultsDir = path.join(process.cwd(), 'indeed-selenium-results');
    await fs.mkdir(resultsDir, { recursive: true });
    
    // Save raw results
    const rawResultsPath = path.join(resultsDir, 'indeed-selenium-raw-results.json');
    await fs.writeFile(rawResultsPath, JSON.stringify(jobs, null, 2));
    logger.info(`Raw results saved to ${rawResultsPath}`);
    
    // Save filtered results
    const filteredResultsPath = path.join(resultsDir, 'indeed-selenium-filtered-results.json');
    ...
    const summaryPath = path.join(resultsDir, 'indeed-selenium-summary.json');
    ...
    logger.error(`Error testing Indeed Selenium scraper: ${error.message}`);
    throw error;
  }
}

// Run the test
testIndeedSeleniumScraper()
  .then(result => {
